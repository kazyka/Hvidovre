...
if (machines_to_select == "CT") {
    ...
    machines_selected <- c("KÊbeskanner")
...
tmp2 <- ddply(tmp, .(tmp$SKS, tmp$year), nrow)
colnames(tmp2) <- c("SKS", "year", "freq")
...
to_match <- c("Haste", "Fremskyndet", "KrÊftpakke", "Rutine", "-1", "DO NOT REMOVE")
...
antal_scan_aab <- alle_r_en_r_aab %>% 
    group_by(Source) %>%
    summarise(Antal_scanninger=n()) 

antal_scan <- alle_r_en_r %>% 
    group_by(Source) %>%
    summarise(Antal_scanninger=n()) 
...
Oversigt af antal scanninger fordelt p≈ scanner ID og prioritet i ≈bningstiden.
Dette er for 2018 og 2019

```{r}
d <- inner_join(n_aabningsdage, antal_scan_aab, by="Source") %>% 
    mutate(scanninger_pr_aabningsdag = Antal_scanninger/Antal_aabningsdage) %>% 
    filter(Source!="Ingen ID") 

colnames(d) <- c("Source", "Antal ≈bningsdage", "Antal lukke dage", 
                 "Antal_scanninger_aab", "Scanninger/≈bningsdag")

d <- inner_join(d, antal_scan, by="Source")

colnames(d) <- c("Source", "Antal ≈bningsdage", "Antal lukke dage", 
                 "Antal_scanninger_aab", "Scanninger/≈bningsdag", "Antal_scanninger")
...
## Antal scanninger fordelt p≈ prioritet og scanner


```{r}
title_text <- paste0("Antal scanninger fordelt p≈ prioritet og scanner; alle dage i ", paste0(c(years_to_select), collapse = "_"))
...
title_text <- paste0("Antal scanninger fordelt p≈ prioritet og scanner i ", paste0(c(years_to_select), collapse = "_"))
...
p <- alle_r_en_r_aab %>% 
    ...
    coord_flip() +
    labs(title=title_text,
         subtitle="≈bningstiden i hverdage", 
         x="Scanner",
         y="Antal",
...
```{r}
title_text <- paste0("Antal scanninger fordelt p≈ prioritet og scanner i lukkettid i ", paste0(c(years_to_select), collapse = "_"))
...
p <- lukket_df %>% 
    ...
    coord_flip() +
    labs(title=title_text,
         subtitle = "Lukketid er alt som ikke er i ≈bningstiden",
         x="Scanner",
         y="Antal",
         fill="") + scale_fill_manual(values = farver)+
...
tmp_gather <- alle_r_en_r %>%
    ...
    filter(!weekday2 %in% c("l¯", "s¯") & !date %in% helligdag) %>%
...
```{r}
title_text <- paste0("Gns. antal haste bes¯g fordelt p≈ tid p≈ dagen og ugedag i ", paste0(c(years_to_select), collapse = "_"))


count_haste_ford <- alle_r_en_r %>% 
    filter(!weekday2 %in% c("l¯", "s¯") & !date %in% helligdag18) %>% 
...
## overblik over haste scanninger fordelt p≈ tid p≈ dagen
...
```{r}
title_text <- paste0("Antal scanninger fordelt p≈ prioritet i ", paste0(c(years_to_select), collapse = "_"))
...
p <- alle_r_en_r_aab %>% 
    ...
    labs(y=NULL, x=NULL, 
         title = title_text,
         subtitle = "Ser kun p≈ ≈bningstiden") +
...
# Scanninger fordelt p≈ prioritet p≈ dagen
...
title_text <- paste0("Antal scanninger fordelt p≈ prioritet og tid p≈ dagen i ", paste0(c(years_to_select), collapse = "_"))
...
title_text <- paste0("Antal skanninger i ≈bningstiden i ", paste0(c(years_to_select), collapse = "_"))
...
subtitle_text <- paste0("Antal skanninger fordelt p≈ prioritet og tid p≈ dagen i ", paste0(c(years_to_select), collapse = "_"))
fordelt_paa_scanner_og_tid <- hverdage %>% 
    filter(Procedure_priority!="-1") %>%
    group_by(Source) %>% 
    nest() %>% 
...
p <- hverdage %>%
    ...
    labs(y="Antal", x="Tid p≈ dagen", title=title_text, fill="") +
...
# fordelt p≈ prioritet og tid p≈ dag / scanner

```{r}

## Fordel p≈ prioritet og tid p≈ dag / skanner
# Fix
ts_version <- alle_r_en_r_aab %>% 
    ...
    mutate(plot = map2(Source, counts, ~.y %>% 
                           ...
                           labs(title= paste("For scanner:", .x, "- hverdage"),
                                subtitle = "Antal scanninger fordelt p≈ prioritet og tid p≈ dagen", 
                                y="Antal", 
                                x="Tid p≈ dagen", 
...
subtitle_text <- paste0("Fordelt p≈ tid i ", paste0(c(years_to_select), collapse = "_"))

if (machines_to_select != "kaebeskanner") {
    tid_plots <- tid_plots %>% 
        gather(Haste, Fremskyndet, `KrÊftpakke`, Rutine, key="Procedure_priority", value="n") %>% 
        group_by(Procedure_priority) %>% 
        nest() %>% 
...
p <- alle_r_en_r_aab %>% 
    ...
    labs(x="SKS",
         y="%",
         title= paste0("Top", " ", nWanted, " ", "SKS koder i ≈bningstiden i ", paste0(c(years_to_select), collapse = "_") )) 
...
colnames(combined_view) <- c("SKS", "Antal i ≈bningstid", paste0(c(years_to_select), collapse = "_"))
...
variation_i_antal_pr_dag <- alle_r_en_r_aab %>% 
    ...
    mutate(plot = map2(.x= antal_pr_dag, .y = Procedure_priority, 
                       ~ .x %>%  
                           ...
                           labs(title=paste0(.y, ": Antal pr. dag i ≈bningstiden", sep=""),
                                x="",
...
for (i in 1:length(variation_i_antal_pr_dag)){
...
variation_i_antal_week <- alle_r_en_r_aab %>% 
    ...
    mutate(plot = map2(.x= antal_pr_uge, .y = Procedure_priority, 
                       ~ .x %>%  
                           ...
                           labs(title=paste0(.y, ": Antal pr. uge i ≈bningstiden", sep=""),
                                x="",
                                y="", 
                                caption= "Bruger 44 uge hvor alle uge er med for at undg≈ bias.") +
...
p <- alle_r_en_r_aab %>% 
    ...
    coord_flip() +
    labs(x="", y="Antal", title="Antal scanninger pr. uge i ≈bningstiden") +
